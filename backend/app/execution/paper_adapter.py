...
class PaperExecutionAdapter(ExecutionEngine):
    ...
    def slippage_bps_for(self, request: ExecutionRequest) -> float:
        """Adverse slippage (bps) for this order (shared model, see fillmodel)."""
        return slippage_bps(
            request.order_kind, request.reference_price * request.quantity,
    ...
    async def submit_order(self, request: ExecutionRequest) -> ExecutionResult:
        ...
        filled_price = slipped_price(
            ...
            bps=self.slippage_bps_for(request),
        )
        ...
        return ExecutionResult(
            ...
            raw_response={
                ...
                "slippage_bps": self.slippage_bps_for(request), "requested_quantity": request.quantity,
            },
        )


def new_client_order_id(agent_id: str, decision_id: str, action: str = "entry") -> str:
    """Deterministic idempotency key: same (agent, decision, action) always
    yields the same order id, so a retried call cannot double-fill."""
